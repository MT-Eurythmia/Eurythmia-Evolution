...
replacer.replace = function( itemstack, user, pointed_thing, mode )
       ...
       if( node.name and node.name ~= "" and replacer.blacklist[ node.name ]) or (def and def.protected) then
       ...
       for list, _ in pairs(inv:get_lists()) do
	       ...
	       if not inv:is_empty(list) then
		       minetest.chat_send_player(name, "Will not replace a node with a non-empty inventory.")
		       return nil
	       end
       end

       if( replacer.blacklist[ daten[1] ]) or (def and def.protected) then
